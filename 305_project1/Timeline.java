...
public class Timeline
{
    ...
    /**
     * find and send the next seading events in the event queue
     * @return
     */
    public Event getNextSendEvent() {
    	PriorityQueue<Event> Copy = new PriorityQueue<Event>(events);
    	while(!Copy.isEmpty()){
    		Event e = Copy.poll();
    	    if(e.getType()==0) {
    	    	return e;
    ...
    /**
     * find and send the next timeout events in the event queue
     * @return
     */
    ...
    /**
     * get the next event but not pull it
     * @return next event
     */
    ...
    public Event returnNextEvent()
    {
        ...
        /*if(tmp.getType()==Event.MESSAGEARRIVE && tmp.pkt.getAcknum()>0 &&  tmp.pkt.getAcknum()-1<sentSoFar){
        	
        	
        	System.out.println("Need to send lost packet "+ sentSoFar);
        }*/
     

        return tmp;
    }
    public void setSendSoFar(int n) {
    	sentSoFar=n;
    }
